...
router.post('/prompts/:id/rollback',requireAiGovernance,async(req,res)=>{try{return res.json({ok:true,correlation_id:req.correlationId,...await prompts.rollback(req.tenant.id,Number(req.params.id),{decidedBy:req.user?.id,correlationId:req.correlationId,reason:req.body?.reason||'production prompt rollback'})});}catch(error){return res.status(error.status||500).json({ok:false,error:error.code||error.message||'AI_PROMPT_ROLLBACK_FAILED',correlation_id:req.correlationId});}});
