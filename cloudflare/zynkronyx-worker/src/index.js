...
export default { async fetch(request,env) {
  ...
  if(path==="/api/capabilities") return json({ok:true,capabilities:[{name:"health",method:"GET",path:"/health",status:"active"},{name:"status",method:"GET",path:"/api/status",status:"active"},{name:"capabilities",method:"GET",path:"/api/capabilities",status:"active"},{name:"gateway-proxy",status:backendUrl(env)?"active":"awaiting-backend-url",paths:["/auth/*","/sync/*","/integration/*","/audit/*","/admin/*"]},{name:"database",status:"active",target:"Firebird"}],timestamp:now()},200,request);
