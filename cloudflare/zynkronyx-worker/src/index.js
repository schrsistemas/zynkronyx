...
function json(data,status=200,request) {
  return new Response(JSON.stringify(data,null,2),{status,headers:{"content-type":"application/json; charset=UTF-8","cache-control":"no-store",...corsHeaders(request.headers.get("Origin")||"")}});
}
function now(){return new Date().toISOString();}
function isGatewayPath(path){return GATEWAY_PREFIXES.some(prefix=>path===prefix||path.startsWith(prefix+"/"));}
function forwardedHeaders(request){
  const headers=new Headers();
  for(const name of ["authorization","content-type","x-api-key","x-device-id","x-device-credential","x-correlation-id"]){
    const value=request.headers.get(name); if(value) headers.set(name,value);
  }
  return headers;
}
async function proxy(request,env,path){
  if(!env.BACKEND_URL)return json({ok:false,error:"BACKEND_NOT_CONFIGURED",version:VERSION},503,request);
  const target=new URL(env.BACKEND_URL.replace(/\/+$/,"")+path+new URL(request.url).search);
  const init={method:request.method,headers:forwardedHeaders(request),redirect:"manual"};
  if(request.method!=="GET"&&request.method!=="HEAD")init.body=request.body;
  try{
    const upstream=await fetch(target,init);
    const headers=new Headers(upstream.headers); headers.set("cache-control","no-store");
    for(const [key,value] of Object.entries(corsHeaders(request.headers.get("Origin")||"")))headers.set(key,value);
    return new Response(upstream.body,{status:upstream.status,statusText:upstream.statusText,headers});
  }catch(error){
    return json({ok:false,error:"BACKEND_UNREACHABLE",detail:error instanceof Error?error.message:String(error),version:VERSION},502,request);
  }
}
function publicResponse(request,path){
  if(path==="/")return new Response(HTML,{headers:{"content-type":"text/html; charset=UTF-8","cache-control":"no-store",...corsHeaders(request.headers.get("Origin")||"")}});
  if(path==="/health")return json({ok:true,service:"zynkronyx-gateway",status:"healthy",version:VERSION,backend:"configured",timestamp:now()},200,request);
  if(path==="/api/status")return json({ok:true,service:"zynkronyx-gateway",version:VERSION,runtime:"cloudflare-workers",backend:"configured",timestamp:now()},200,request);
...
export default {async fetch(request,env){
  const url=new URL(request.url); const path=url.pathname.replace(/\/+$/,"")||"/"; const origin=request.headers.get("Origin")||"";
  if(request.method==="OPTIONS")return new Response(null,{status:204,headers:corsHeaders(origin)});
  if(PUBLIC_PATHS.has(path))return publicResponse(request,path);
  if(isGatewayPath(path))return proxy(request,env,path);
