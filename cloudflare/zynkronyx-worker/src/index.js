...
function publicHeaders(request) {
  ...
  const headers = {
    ...
    "access-control-allow-headers": "Authorization,Content-Type,X-API-Key,X-Device-Id,X-Device-Credential,X-Correlation-Id,Idempotency-Key,X-Zynkronyx-Tenant",
...
export default { async fetch(request,env) {
  ...
  if(path==="/api/capabilities") return json({ok:true,capabilities:[{name:"health",method:"GET",path:"/health",status:"active"},{name:"status",method:"GET",path:"/api/status",status:"active"},{name:"capabilities",method:"GET",path:"/api/capabilities",status:"active"},{name:"gateway-proxy",status:backendUrl(env)?"active":"awaiting-backend-url",paths:["/auth/*","/sync/*","/integration/*","/audit/*","/admin/*","/metrics/*","/ai/*","/sales/*","/fiscal/*"]},{name:"database",status:"configured",target:"transactional-sgbd"},{name:"ai-sales",status:"active",paths:["/sales/leads","/sales/opportunities","/sales/opportunities/:id/copilot"]}],timestamp:now()},200,request);
